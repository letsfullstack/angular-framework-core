(function () {
	...
	angular.module('letsAngular').directive('fwMultiSelect', fwMultiSelect);
		...
		return {
			...
			controller: function ($scope, $state) {
				...
				$scope.$on('filter-init', function (scope) {
					$scope.restartMultiSelect(scope);
				});

				$scope.restartMultiSelect = function (scope) {
				...
				$scope.$on('filter-rest', function (scope) {
					$timeout(function () {
						$scope.msmodel = [];
					});
				});
				...
				$scope.makeRequestAutocomplete = function (scope, value, route, isGeneral) {
					$timeout(function () {
						...
						$scope.resource.customGET(prefix + 'autocomplete/' + route + '/' + value + '?limit=10').then(
							function (data) {
				...
				$scope.onInitMulti = function (event, field) {
					...
					if (!dropdown.initMultiSelect) {
						...
						_scope.externalEvents.onDeselectAll = $scope.onDeselectAll;
