  ...
  module.controller('CRUDController', function ($scope, Restangular, module, $state, $window, $stateParams, $rootScope, headers, swangular, ngToast) {
    ...
    $scope.export = function () {
          ...
          fields.forEach(function (campo) {
            ...
            if (campo.exportColumn) {
              ...
              if (Array.isArray(campo[nextfield[0]])) {
                ...
                var tmp = caminho.split('#').length;
                if (tmp > 2) {
                  trocas.push({
                    antes: [caminho.split('#')[0], caminho.split('#')[1]],
                    depois: nextfield[0],
                  });
                }
              }
      ...
      function extraiColunas(headers) {
        var headers_field = headers.fields.filter(filtraExport).map(mapExportName);
        if (headers.tabs) {
      ...
      function percorreObjeto(objeto, caminho) {
        ...
        return caminho.reduce(function (obj, key) {
          // if (key == 'card_params') debugger;
          // if (key == 'cards') debugger;
        ...
        .then(function (result) {
          ...
          if (!!result.value && !!$scope.totalPager && $scope.totalPager != undefined) {
            ...
            resource.customGET('pager', all_filter).then(function (response) {
              // console.log(response.data, clmnxlsx)
              function nextBeforeExport() {
                ...
                res.map(function (row) {
                  ...
                  trc.forEach(function (troca) {
                    ...
                    // if (!aux) debugger;
                    // console.log({ a: aux, b: row, c: troca, d: trocas, e: fields });
                    if (typeof aux === 'object') aux = aux.join(', ');
                    // else if (typeof (aux) === 'string') aux = aux.join(', ');
                    // debugger
